...
- **图片上传**：直接 **粘贴** 或 **拖拽** 图片到编辑框，自动上传至 R2 并插入 `![](url)`
- **标签补全**：输入 `#` 自动弹出已有标签候选，↑↓ 选择、Enter 确认

### 🗒 笔记管理

- ⏱ **时间流**：按「今天 / 昨天 / 日期」分组展示，无限滚动加载
- 🏷 **标签系统**：`#标签` 即写即建，支持中文与 `#项目/子项目` 嵌套标签，侧栏统计数量
- 🔍 **全文搜索**：毫秒级检索所有笔记内容
- ⭐ **收藏**：重要笔记一键收藏，独立视图查看
- 🎲 **随机回顾**：随机抽取历史笔记，像 flomo 一样「每日回顾」
...
- 一键导出全部笔记为 **Markdown** 或 **JSON** 文件
- 数据库结构自动迁移，部署即用，无需手动初始化
...
│   ├── index.js          # 路由与全部 API（笔记 CRUD / 标签 / 搜索 / 上传 / 导出）
│   ├── auth.js           # PBKDF2 密码散列 + D1 会话管理
│   ├── tags.js           # #标签 提取规则（支持中文与嵌套）
│   └── migrate.js        # 数据库结构自动迁移
├── public/               # 前端（零构建，原生 ES Module）
│   ├── index.html
│   ├── assets/
│   │   ├── app.js        # 应用主体（列表 / 弹窗 / 主题 / 路由）
│   │   ├── editor.js     # Markdown 编辑器组件
│   │   ├── md.js         # Markdown 渲染 + 标签链接化
│   │   ├── api.js        # 请求封装
│   │   └── style.css     # flomo 风格样式（浅色 / 深色主题）
