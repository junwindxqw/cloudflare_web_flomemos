...
- **完整工具栏**：加粗、斜体、删除线、H1/H2/H3、引用、无序/有序/任务列表、行内代码、代码块、链接、图片、表格、分割线；再次点击可切换取消，标题层级间点击直接替换
- **实时预览**：一键在「编辑」与「预览」之间切换，渲染效果与列表一致；预览中可勾选任务复选框，自动反写 Markdown 源码
- **全屏编辑（Zen）**：一键占满整个窗口，右侧**目录（TOC）**实时生成，编辑 / 阅读模式下都可点击跳转、滚动自动高亮当前章节，`Esc` 退出
- **智能续写**：回车自动延续列表（`- `、`1. `、`- [ ] `）与引用，空列表项回车自动退出；`Tab / Shift+Tab` 对列表行缩进 / 反缩进
- **快捷键**：`Ctrl+B` 加粗 · `Ctrl+I` 斜体 · `Ctrl+K` 链接 · `Ctrl+E` 行内代码 · `Ctrl+Shift+X` 删除线 · `Ctrl+Alt+1/2/3` 标题 · `Ctrl+Shift+7/8/9` 列表 · `Ctrl+Enter` 发送
- **图片上传**：直接 **粘贴** 或 **拖拽** 图片到编辑框，> 1MB 自动压缩后上传至 R2 并插入 `![](url)`
- **上传进度**：上传时显示百分比
- **自动保存草稿**：写到一半刷新不丢，提交后自动清除
- **标签补全**：输入 `#` 自动弹出已有标签候选（`startsWith` 优先），↑↓ 选择、Enter 确认
- **归档目录**：编辑器底部可选目录，新建即归档；按目录视图写作时自动带上当前目录

### 🗒 笔记管理

- ⏱ **时间流**：按「今天 / 昨天 / 日期」分组展示，无限滚动加载
- 📂 **目录树**：与 #标签 相互独立的层级体系，支持多级子目录；右键新建子目录 / 重命名 / 移动到… / 删除（笔记自动变为未归类，子目录上移）；「未归类」一键筛出未整理的笔记
- 🏷 **标签系统**：`#标签` 即写即建，支持中文与 `#项目/子项目` 嵌套标签，侧栏搜索过滤 + 三种排序（频率/名称/最近使用）+ 右键重命名 / 合并 / 删除
- 📜 **长文自动折叠**：内容过长的笔记自动折叠（底部渐隐 + 展开全文按钮），点开再收起都很顺手
...
| `Ctrl + E` | 行内代码 |
| `Ctrl + Shift + X` | 删除线 |
| `Ctrl + Alt + 1/2/3` | 一 / 二 / 三级标题 |
| `Ctrl + Shift + 7/8/9` | 有序 / 无序 / 任务列表 |
| `Enter` | 列表 / 引用自动续写 |
| `Tab` / `Shift + Tab` | 列表行缩进 / 反缩进（普通行插入两格） |
| `Esc` | 退出全屏编辑 |
| `#` | 触发标签自动补全 |
| 粘贴 / 拖拽图片 | 自动上传并插入 |
...
│   ├── assets/
│   │   ├── app.js        # 应用主体（列表 / 目录 / 弹窗 / 主题 / 路由 / 统计）
│   │   ├── editor.js     # Markdown 编辑器（压缩 / 续写 / 标签补全 / 草稿 / 全屏 TOC）
│   │   ├── editorCore.js # 编辑器纯函数（TOC 标题解析 / 工具栏行变换）
│   │   ├── folderTree.js # 目录树构建纯函数（id/parent_id → 层级树）
│   │   ├── tagTree.js    # 标签树构建纯函数（#a/b 路径 → 层级树）
...
<details>
<summary><b>目录和标签是什么关系？</b></summary>

两套完全独立的层级体系：**目录**是笔记的归属地（一条笔记只属于 0 或 1 个目录，侧栏树状管理，适合「笔记本」式整理）；**标签**写在正文里（一条笔记可带多个，支持 `#a/b` 嵌套，适合横向打维度）。两者可以叠加使用，互不影响。
</details>

<details>
<summary><b>删除目录会丢笔记吗？</b></summary>

不会。删除目录只是解除归属：目录下的笔记自动变为「未归类」，子目录整体上移一级，笔记本身与标签完全不动。
</details>
